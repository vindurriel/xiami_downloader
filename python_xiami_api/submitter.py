...
def upload(path,fname):
	...
	url_file=  "https://c.pcs.baidu.com/rest/2.0/pcs/file"
...
def push():
	...
	def allows(x):
		...
		return ext not in ['.txt','.pdb','.log','.zip','.suo','.dat']
	for f in filter(allows,os.listdir(f_xiami)):
		z.write(os.path.join(f_xiami,f),f)
	z.close()
	path="/apps/folder1/"
	status=upload(path+"version.txt",f_version)
	status=upload(path+"latest.zip",f_zip)
	if status==200:
		os.remove(f_version)
		os.remove(f_zip)
push()