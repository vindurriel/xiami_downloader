...
public class XiamiSearchProvider : ISearchProvider
{
    ...
    static List<IMusic> GetSongsOfArtist(string json)
    {
        ...
        try
        {
            var obj = JsonParser.Parse(json).GetObject()["songs"].GetArray();
    ...
    static List<IMusic> GetSongsOfCollect(string json)
    {
        ...
        try
        {
            var obj = JsonParser.Parse(json).GetObject()["collect"].GetObject()["songs"].GetArray();
            foreach (var x in obj)
            {
                items.Add(MusicFactory.CreateFromJson(x, EnumMusicType.song));
            }
    ...
    static List<IMusic> GetSongsOfAlbum(string json)
    {
        ...
        {
            var obj = JsonParser.Parse(json).GetObject();
            var album_name = obj["album"].GetObject()["title"].GetString();
            foreach (var x in obj["album"].GetObject()["songs"].GetArray())
            {
                Song a = MusicFactory.CreateFromJson(x, EnumMusicType.song) as Song;
    ...
    async static Task<Song> GetSong(string id)
    {
        ...
        try
        {
            ...
            ////////
            if (json == null) return null;
            var obj = JsonParser.Parse(json).GetObject()["song"].GetObject();
            song = MusicFactory.CreateFromJson(obj, EnumMusicType.song) as Song;
        }
        catch { }
        return song;
    }
    public async static Task<Album> GetAlbum(string id)
    {
        ...
        var json = await NetAccess.DownloadStringAsync(url);
        if (json == null) return null;
        var obj = JsonParser.Parse(json).GetObject()["album"];
        var Album = MusicFactory.CreateFromJson(obj, EnumMusicType.album) as Album;
        return Album;
    }
}